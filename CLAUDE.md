...
- `period.js` —— 生理期 / 周期 共享逻辑(替代已删的 `cycle.js`)。`computePeriodStatus(cycleConfig, todayDk?)` 5 phase(`inactive` / `in-period` / `fluctuation` / `predicted` / `overdue`)+ `findPeriodType(types)`(从 checkinTypes 找 kind='period' 那条)+ `addDays / daysBetween / dayKeyToTs` 日期算术。**数据源 T23 后换了**:不再读独立 `cycle` 单例,而是 `checkinTypes` 里 kind='period' 那条 type 的 `cycleConfig` 字段。cycle-app / cycle-notify / context 都 import 它。
...
- `ai.js` —— `callAIOnce / callAI / requestReply` 三层。tool calling 支持 `get_current_time` / `get_weather` / `get_location`(按 chatSessions 的 toggle 启用)。`requestReply` 顶部有 `Map<sessionId, Promise>` in-flight 锁(防多 tab 共享 IDB 时并发触发记忆重复 archive),末尾调一次 `maybeCompressMemory`。**暂停生成**:平行的 `Map<sessionId, AbortController>` + 导出 `abortReply(sessionId)` — chat.js 在 AI 生成中再按「让 AI 回复」(此时按钮加 `.generating` 变红、title 改「停止生成」)就调它中断本次请求(`signal` 透传 requestReply → callAIOnce → fetch);AbortError 当「已停止」静默处理不弹错,中断点在 fetch、此前没落库所以无半条残留。
...
| `cycle` | `id` | — | **DB_VERSION 13 加,T23 起废弃**。原生理期单例。**T23 迁移**(`main.js#migrateCycleToCheckin`)把数据搬到 `checkinTypes` 里 kind='period' 那条 type 的 `cycleConfig` 字段(同 shape:enabled / visibleToChat / averageLength / periodLength / fluctuation / lastStartDayKey / history),并删掉本 store 的 row。store 定义保留兼容,但 cycle-app / context / cycle-notify 都已改读 `checkinTypes` 的 period 配置。**双门控**:enabled + visibleToChat 都开 prompt 才注入(隐私敏感)。预测算法 `core/period.js#computePeriodStatus`。period 是特殊 checkinType,被 `buildCheckinLines` 和 schedule app 显式过滤(不混进普通打卡、不从打卡摘要泄漏) |
...
13. **删文件 / 不可逆操作必须先经用户明确同意**:Claude **不得擅自删除或移动用户电脑上的文件**(`rm` / `del` / `git rm` / 重命名搬走 / 把目录清空 等),哪怕它看起来是死代码、是重构的"顺手清理"、或刚被某个改动替代。**改 / 写 / 新增文件不在此列** —— 那是正常开发活儿,照常做。**边界**:可逆的内容修改(Edit / Write 某个文件)= 允许;**不可逆的删除 / 移动 / 把整文件覆盖成空 = 必须先问,等用户明确说"删"再动手**。**Why**:删文件不可逆,用户对自己机器上有什么文件有最终决定权;"它是死代码"是 Claude 的判断,不该替用户拍板。发现可删的死代码就指出来、建议删,然后**等用户点头**。(本条因 `core/cycle.js` 被擅自 `rm` 而立。)
...
- ✅ **T9-T11 生理期 / 周期 app**(全新功能):新 store `cycle` + `cycleSymptoms` / `core/cycle.js` 算法 / `core/cycle-notify.js` boot 通知 / `features/cycle/cycle-app.js` 3 tab UI / `buildCycleStatus` prompt 注入双门控。**后续**:T23 把数据迁到 `checkinTypes`(kind='period').cycleConfig;cycle-A 把 cycle-app 重接到该数据源、算法换成 `core/period.js`、删掉 `core/cycle.js`、schedule + buildCheckinLines 过滤 period(隐私)
